...
test := []Test{
		...
		Test{ItemName:"orange",NameOfPerson:"SUJITH",SellingValue:"120"},
		Test{ItemName:"grapes",NameOfPerson:"SALEEL",SellingValue:"190"},
	}
	i := 0
	for i < len(test) {
		fmt.Println("i is ", i)
		testAsBytes, _ := json.Marshal(test[i])
		stub.PutState("ITEM"+strconv.Itoa(i), testAsBytes)
...
func (t *HeroesServiceChaincode) invoke(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	if len(args) < 2 {
		...
		var test = Test{ItemName:newProduct.ItemName,NameOfPerson:newProduct.NameOfPerson,SellingValue:newProduct.SellingValue}
